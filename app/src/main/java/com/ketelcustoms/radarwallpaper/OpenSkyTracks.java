...
final class OpenSkyTracks {
    ...
    static boolean attachCached(Context context,List<FlightCalendar.Leg> legs){boolean changed=false;for(FlightCalendar.Leg leg:legs){List<double[]> points=readTrack(trackFile(context,leg));if(points!=null){leg.actualTrack=points;changed=true;}}return changed;}
    ...
    static boolean downloadMissing(Context context,SharedPreferences prefs,List<FlightCalendar.Leg> legs,long now){
        ...
        String token=null;boolean changed=false;int attempted=0;for(FlightCalendar.Leg leg:legs){if(leg.start>=now||leg.actualTrack!=null||leg.fromIcao.isEmpty()||leg.toIcao.isEmpty())continue;File miss=missFile(context,leg);if(miss.isFile()&&now-miss.lastModified()<RETRY_DELAY)continue;if(attempted++>=3)break;
            try{if(token==null)token=token(client,secret);JSONObject flight=findFlight(token,leg);if(flight==null){markMiss(miss);continue;}List<double[]> points=fetchTrack(token,flight.optString("icao24"),(flight.optLong("firstSeen")+flight.optLong("lastSeen"))/2);if(points==null||points.size()<2){markMiss(miss);continue;}writeTrack(trackFile(context,leg),points);if(miss.isFile())miss.delete();leg.actualTrack=points;changed=true;}catch(Exception ignored){markMiss(miss);}
        }return changed;
    }

    static int cachedCount(Context context,List<FlightCalendar.Leg> legs){int count=0;for(FlightCalendar.Leg leg:legs)if(trackFile(context,leg).isFile())count++;return count;}

    private static JSONObject findFlight(String token,FlightCalendar.Leg leg)throws Exception{
        long begin=(leg.start-8L*60L*60L*1000L)/1000L,end=(leg.start+8L*60L*60L*1000L)/1000L;String url="https://opensky-network.org/api/flights/departure?airport="+encode(leg.fromIcao)+"&begin="+begin+"&end="+end;JSONArray flights=new JSONArray(get(url,token));JSONObject best=null;double bestScore=-1e9;String wantedDigits=leg.flightNumber.replaceAll("\\D","");
        for(int i=0;i<flights.length();i++){JSONObject flight=flights.getJSONObject(i);String departure=flight.optString("estDepartureAirport"),arrival=flight.optString("estArrivalAirport"),callsign=flight.optString("callsign","").trim();long first=flight.optLong("firstSeen")*1000L;double hours=Math.abs(first-leg.start)/3600000.0;double score=-hours;if(leg.fromIcao.equals(departure))score+=5;if(leg.toIcao.equals(arrival))score+=8;String foundDigits=callsign.replaceAll("\\D","");if(!wantedDigits.isEmpty()&&wantedDigits.equals(foundDigits))score+=8;if(!leg.flightNumber.isEmpty()&&leg.flightNumber.equals(callsign.replaceAll("\\s+","")))score+=4;if(score>bestScore){bestScore=score;best=flight;}}
        return bestScore>=9?best:null;
    }

    private static List<double[]> fetchTrack(String token,String icao24,long time)throws Exception{if(icao24==null||icao24.isEmpty()||time<=0)return null;JSONObject track=new JSONObject(get("https://opensky-network.org/api/tracks/all?icao24="+encode(icao24.toLowerCase(Locale.US))+"&time="+time,token));JSONArray path=track.optJSONArray("path");if(path==null)return null;ArrayList<double[]> points=new ArrayList<>();for(int i=0;i<path.length();i++){JSONArray point=path.optJSONArray(i);if(point==null||point.length()<3||point.isNull(1)||point.isNull(2))continue;double lat=point.optDouble(1,Double.NaN),lon=point.optDouble(2,Double.NaN);if(Double.isFinite(lat)&&Double.isFinite(lon))points.add(new double[]{lat,lon});}return points;}
    ...
    private static File missFile(Context context,FlightCalendar.Leg leg){return new File(folder(context),key(leg)+".miss");}
