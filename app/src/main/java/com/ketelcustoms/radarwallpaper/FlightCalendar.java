...
final class FlightCalendar {
    ...
    static boolean routeFits(Leg leg,double centerLat,double centerLon,int zoom,int bitmapW,int bitmapH,int surfaceW,int surfaceH){if(leg==null||bitmapW<1||bitmapH<1||surfaceW<1||surfaceH<1)return false;double world=256.0*(1<<zoom),cx=worldX(centerLon,world),cy=worldY(centerLat,world);float margin=Math.max(6f,bitmapW*.025f);for(int i=0;i<=32;i++){double[] point=routePoint(leg,i/32f);float x=screenX(point[1],world,cx,bitmapW,surfaceW),y=(float)(((worldY(point[0],world)-cy)*bitmapH/surfaceH)+bitmapH/2.0);if(x<margin||x>bitmapW-margin||y<margin||y>bitmapH-margin)return false;}return true;}
