...
public class SettingsActivity extends Activity {
    ...
    private void addOpenSkySection(LinearLayout root,boolean calendarReady){
        ...
        if(calendarReady){List<FlightCalendar.Leg> legs=FlightCalendar.load(this,prefs,System.currentTimeMillis());int exact=OpenSkyTracks.exactCount(this,legs),borrowed=OpenSkyTracks.borrowedCount(this,legs);TextView cached=text("Tracks cached: "+exact+" exact, "+borrowed+" borrowed from an earlier flight",13);cached.setTextColor(Color.rgb(154,174,183));root.addView(cached);int flightCredits=prefs.getInt("opensky_flight_credits",-1),trackCredits=prefs.getInt("opensky_track_credits",-1);if(flightCredits>=0||trackCredits>=0){String quota="Credits remaining — flights: "+(flightCredits<0?"unknown":flightCredits)+", tracks: "+(trackCredits<0?"unknown":trackCredits);TextView quotaStatus=text(quota,13);quotaStatus.setTextColor(Color.rgb(154,174,183));root.addView(quotaStatus);}String check=prefs.getString("opensky_last_status","Not checked yet.");TextView checkStatus=text("Last check: "+check,13);checkStatus.setTextColor(Color.rgb(154,174,183));root.addView(checkStatus);if(configured){Button retry=new Button(this);retry.setText("Check OpenSky now");retry.setOnClickListener(v->{retry.setEnabled(false);retry.setText("Checking OpenSky…");new Thread(()->{List<FlightCalendar.Leg> current=FlightCalendar.load(this,prefs,System.currentTimeMillis());OpenSkyTracks.retryNow(this,prefs,current,System.currentTimeMillis());sendBroadcast(new Intent(RadarWallpaperService.ACTION_REFRESH_TRACKS).setPackage(getPackageName()));mainHandler.post(this::render);},"opensky-check").start();});root.addView(retry);}}
