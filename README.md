...
- The settings screen provides **Check OpenSky now** and reports authentication, rate-limit, timeout, no-match or successful-download results instead of silently suppressing failures. Automatic no-match retries wait one hour.
...
- When the phone is unlocked, completed flights touching the most recent local travel date animate chronologically. A flight departing Friday and landing Saturday is therefore followed by a second Saturday flight. Long-haul routes animate along their final portion; when there are multiple flights and the final flight fits completely on the map, that final flight animates from departure to arrival. The normal wallpaper appears for one second between flights. A bright dot in the selected trail colour marks the moving aircraft. Unlock requests are retained until the wallpaper becomes visible and Android reports that the keyguard is open; radar refreshing waits until the sequence is complete. Both recorded ADS-B tracks and great-circle fallbacks animate. These animations reuse the cached wallpaper frame and perform no radar downloads.
