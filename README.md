...
- When the phone is unlocked, the cached map and radar first appear without any flight paths. Completed flights touching the most recent local travel date then animate chronologically, with a bright aircraft dot progressively drawing each path behind it. A flight departing Friday and landing Saturday is therefore followed by a second Saturday flight; the first completed path remains visible during the one-second pause while the next path is still hidden. Long-haul routes animate along their final portion; when there are multiple flights and the final flight fits completely on the map, that final flight animates from departure to arrival. Unlock requests are retained until the wallpaper becomes visible and Android reports that the keyguard is open; radar refreshing waits until the sequence is complete. Both recorded ADS-B tracks and great-circle fallbacks animate. These animations reuse the cached wallpaper frame and perform no radar downloads.
