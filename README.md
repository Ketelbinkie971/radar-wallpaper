...
- The settings screen provides **Check OpenSky now** and reports authentication, rate-limit, timeout, no-match or successful-download results instead of silently suppressing failures. It also shows the remaining flight and track credits returned by OpenSky.
- OpenSky usage is credit-conscious: airport/day flight lists are cached and reused, only the newest eligible uncached flight is queried per pass, unsuccessful flights wait 24 hours, repeated manual checks are blocked for one hour, and HTTP 429 retry windows are respected.
...
- When the phone is locked, the wallpaper immediately replaces the visible flight paths with its locally cached clean map and radar frame. On unlock, completed flights touching the most recent local travel date animate chronologically, with a bright aircraft dot progressively drawing each cached path behind it. A flight departing Friday and landing Saturday is therefore followed by a second Saturday flight; the first completed path remains visible during the one-second pause while the next path is still hidden. Long-haul routes animate along their final portion; when there are multiple flights and the final flight fits completely on the map, that final flight animates from departure to arrival. If there is nothing eligible to animate, the normal paths return automatically. Unlock requests are retained until the wallpaper becomes visible and Android reports that the keyguard is open; radar refreshing waits until the sequence is complete. Both recorded ADS-B tracks and great-circle fallbacks animate. Locking and animating use only cached frames and coordinates and perform no RainViewer or OpenSky downloads.
